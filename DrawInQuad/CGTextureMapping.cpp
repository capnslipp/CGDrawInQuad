...
struct DestImageGenInfo {
	...
	union {
		GLKVector2 points[4];
		struct { GLKVector2 pointAftStar, pointAftPort, pointForStar, pointForPort; };
	};
	
	GLKVector2 segmentAftDelta, segmentForDelta;
	float segmentAftLengthSqr, segmentForLengthSqr;
...
{
	GLKVector2 nearestPointOnASegment, nearestPointOnFSegment;
	float ratioAlongA = ratioAndNearestPointAlongSegment(
		surfaceST,
		info->pointAftStar, info->pointAftPort,
		info->segmentAftDelta, info->segmentForLengthSqr,
		&nearestPointOnASegment
	);
	float ratioAlongF = ratioAndNearestPointAlongSegment(
		surfaceST,
		info->pointForStar, info->pointForPort,
		info->segmentForDelta, info->segmentForLengthSqr,
		&nearestPointOnFSegment
	);
	
	GLKVector2 nearestPointOnASegmentUV = GLKVector2Lerp(info->pointUV0, info->pointUV1, ratioAlongA);
	GLKVector2 nearestPointOnFSegmentUV = GLKVector2Lerp(info->pointUV2, info->pointUV3, ratioAlongF);
	float ratioBetweenNearestAAndF = ratioAlongSegment(surfaceST, nearestPointOnASegment, nearestPointOnFSegment);
	
	GLKVector2 texelUV = GLKVector2Lerp(nearestPointOnASegmentUV, nearestPointOnFSegmentUV, ratioBetweenNearestAAndF);
	return texelUV;
}
...
{
	static const int kStarForTriInQuadIndices[3] = { 0, 1, 2 };
	static const int kPortAftTriInQuadIndices[3] = { 1, 3, 2 };
	...
	float lineVsPointCross = GLKVector2CrossProduct(
		GLKVector2Subtract(info->pointForStar, info->pointAftPort),
		GLKVector2Subtract(surfaceST, info->pointAftPort)
	);
	bool inStarForTri = lineVsPointCross > 0.0f;
	
	const int *triInQuadIndices = inStarForTri ? kStarForTriInQuadIndices : kPortAftTriInQuadIndices;
...
CFDataRef createDestImageData(int srcWidth, int srcHeight, CFDataRef srcData, int destWidth, int destHeight, GLKVector2 points[4])
{
	...
	struct DestImageGenInfo info = {
		...
		.pointForStar = points[2],
		.pointForPort = points[3],
	...
	info.segmentForDelta = GLKVector2Subtract(info.pointForPort, info.pointForStar);
	...
	info.segmentForLengthSqr = GLKVector2AllEqualToScalar(info.segmentForDelta, 0.0f) ? FLT_MIN : GLKVector2LengthSqr(info.segmentForDelta);
	...
	memset(byteBuffer, 0xff, pixelCount * kBytesPerPixel);
